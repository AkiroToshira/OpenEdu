...
export default function Tests() {
  ...
  useEffect(async () => {
	const lessons = await axios.get('http://127.0.0.1:8000/tests/lesson/3', {
	  headers: {
  ...
  const handleSubmit = async () => {
    console.log(answers)
	await axios.post('http://127.0.0.1:8000/tests/count/3',{"answers": answers.join(' ')}, {
	  headers: {
		"Authorization": "JWT " + state.auth.token.access,
	  }
	}, ).then((res) => console.log(res,'submited'))
  }
  return (
		...
		<main className="tests_main">
		  <div className="tests_container">
			...
			<div className="tests_tests_container">
			  ...
			  {questions.questions && questions.questions.map((el, i) => {
				return <div key={el.id} className="tests_tests_container_test" onClick={() => openModal(i)}>{i + 1}</div>
			  })}
			</div>
		  </div>
		  <div id="myModal" class="modal" style={{'display': `${close?'none':'block'}`}} >
			<div class="modal-content">
			  ...
			  <h1>{questions.questions && questions.questions[modalId] && questions.questions[modalId].text}<span onClick={() => openModal()}>X</span></h1>
			  <div>
